...
class NLGLIF3(nn.Module):
    def __init__(self,
        ...
        self.K_theta_s = nn.Parameter(-torch.ones_like(self.V_threshold), requires_grad=False)
        self.A_theta_s = nn.Parameter(torch.zeros_like(self.V_threshold), requires_grad=False)
        self.K_theta_v = nn.Parameter(-torch.ones_like(self.V_threshold), requires_grad=False)
        self.A_theta_v = nn.Parameter(torch.zeros_like(self.V_threshold), requires_grad=False)
    ...
    def forward(self, I_ext):
        '''
        calculate the V and S from given external stimulus I_ext using the nonlinear version of the glif3 mdoel.
        Input: 
...
def example():
    nlglif3_paras = {'V_rest': [-75.0,-70.0,-65.0],
    ...
    nlglif3 = NLGLIF3(nlglif3_paras,  # dictionary of parameters {'K_V': [K_V,K_V,....,K_V], 'R_V': ..., ...}
                 dt=0.002,  # this should be the length of a time bin in units of seconds.
                 device='cuda')
    
    S_pred,V_pred = nlglif3(I_ext)
    plt.figure()
